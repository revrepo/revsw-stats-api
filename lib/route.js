...
routes_.PUT[endpoint_stats_ + 'apps'] = function( req, resp ) {
  ...
  req.on( 'end', function() {
    ...
    try {
      ...
      logger.debug( 'ip, msg: ' + ip_, message );
...
routes_.POST[endpoint_ + 'force-keys-reload'] = function( req, resp ) {
  ...
  keys.loadKeys2Redis()
    .then( function( response ) {
      logger.info( 'forced keys reload: pairs loaded ' + response.set + ', deleted ' + response.deleted );
      resp.writeHead( 200, { 'Content-Type': 'application/json' });
      resp.end( JSON.stringify({
        message: 'SDK keys reloaded',
        version: API_version_
      }) );
    })
    .catch( function( err ) {
