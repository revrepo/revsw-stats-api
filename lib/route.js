    ...
    keys.getAccountAppIDs(message.sdk_key)
      .then(function(ids) {
        if (!ids) {
          ...
          _reply(resp, 401, 'Not authorized');
          return;
        }
        ...
        if (dispatcher.handle(message)) {
          ...
          _reply(resp, 400, 'Bad request: wrong data format');
        }
      });
  });
};
...
//  ---------------------------------
//  force-fire-queue, disabled

// channel.on('force-fire-queue', function(msg) {
//   logger.info('got broadcasted "force-fire-queue"');
...
//   channel.broadcast('force-fire-queue');
//   dispatcher.forceFireQueue()
//     .then(function(response) {
//       logger.info('forced fire queue, completed');
//       resp.writeHead(200, {
