  ...
  queue_cleaner_ = setInterval(function() {
    ...
    if (msg_queue_.length > 0) {
      if ( (Date.now() - last_call_) > config.service.queue_clear_timeout) {
        ...
        logger.debug('Dispatcher\'s queue_cleaner_: queue is not yet timed out, ' + (Date.now() - last_call_) );
      }
    } else {
...
var push_ = function(msg) {
  ...
  msg.device.model = device_dict_[(msg.device.device.split(' ')[0])] || '';
  msg_queue_.push({
    index: {
      _index: idx
    }
...
module.exports = {
  ...
  handle: function(message) {
    ...
    message.received_at = last_call_ = Date.now();
