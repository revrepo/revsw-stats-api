...
var push_ = function(msg) {
  ...
  if (msg.version === undefined ||
    ...
    logger.error('Dispatcher: message queue error: wrong data format');
    return false;
  }
  ...
  var now = Date.now();
  if ( msg.hits ) {
    msg.start_ts = msg.requests[0].start_ts;
    msg.end_ts = msg.requests[msg.hits - 1].end_ts;

    index_date = msg.start_ts;
    if ( now - msg.start_ts > config.service.request_max_age ) {
      logger.warn( 'Dispatcher::push_: requests are too old, using now() for the index name' );
      index_date = now;
    }
  } else {
    msg.start_ts = msg.end_ts = -1;
    index_date = now;
  }

  var idx = get_index_(index_date);
