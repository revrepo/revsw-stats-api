...
module.exports = {
  ...
  findOneKey: function() {
    ...
    return new promise( function( resolve, reject ) {
      ...
      mongo.MongoClient.connect( config.service.key_id.db_connection, function( err, db ) {
        ...
        //  load currently actual keys from the far-away mongodb
        db.collection( config.service.key_id.collection )
          .findOne({ deleted: false }, { _id:0, sdk_key: 1, account_id: 1 }, function( err, rec ) {
  ...
  //  and healthcheck
  checkMongoConnection: function() {

    return new promise( function( resolve, reject ) {

      mongo.MongoClient.connect( config.service.key_id.db_connection, function( err ) {
